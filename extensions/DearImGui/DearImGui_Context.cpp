...
{
	DearImGuiContext::DearImGuiContext(const std::shared_ptr<Window>& aWindow, const std::shared_ptr<RenderTexture>& aRenderTarget, std::function<void()> anImGuiRenderCallback)
		: myHasWindow(true)
		...
		, myImGuiRenderCallback(anImGuiRenderCallback)
	{
		...
		DearImGuiBackendContext::CreateBackendHandlers(aWindow, aRenderTarget, myBackends);
	}
	...
	void DearImGuiContext::Render(Atrium::FrameGraphicsContext& aFrameContext)
	{
		...
		for (const auto& backend : myBackends)
			backend->Render(aFrameContext);
	}

	void DearImGuiContext::StyleColorsNord()
	{
